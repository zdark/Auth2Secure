...
// HTML Page Start
htmlHeader($settings[1] . " [Profiles]", $settings_root_site, TRUE); //title, siteRootPath, menuShow
                    ...
                    <form action="#" method="post">
                        <select class="input" name="role">
                            ...
                            <?
                                ...
                                //while($rs_role = mysql_fetch_object($result_role)) { // OLD WHILE DB
                        ...
                        <table border="0" cellspacing="1" cellpadding="1">
                          <tr>
                            ...
                            <?
                                $sql = "SELECT T1.*, T2.* FROM ac_item AS T1
                                        LEFT JOIN ac_mm_role_item AS T2 ON T1.id = T2.item_id AND role_id = $role ";
                                ...
                                //while($rs = mysql_fetch_object($result)) { // OLD WHILE DB
                          ...
                          <tr>
                            <td class="table-cell-1">
                                <? 
                                    if ($rs->role_id == "") {
                                        echo "<a href='admin_profile_action.php?item_id=".encryptData(session_id(), $rs->id)."&action=add'>" . $rs->name_item . "</a>";
                                    }
                                ?>	</td>
                            <td class="table-cell-1">
                                <? 
                                    if ($rs->role_id != "") {
                                        echo "<a href='admin_profile_action.php?item_id=".encryptData(session_id(), $rs->id)."&action=remove'>" . $rs->name_item . "</a>";
                                    }
                                ?>	</td>
                          </tr>
                            <? 
