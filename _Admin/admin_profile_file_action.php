...
$role = $_SESSION["role"];
//$file_id = sec_cleanTAGS($_GET["file_id"]);
$file_id = sec_cleanTAGS(decryptData(session_id(), $_GET["file_id"]));
$action = sec_cleanTAGS($_GET["action"]);

if ($action == "add") {
	$sql = "INSERT INTO ac_mm_role_file (role_id, file_id) VALUES ($role,$file_id)";
	...
	$sql = "DELETE FROM ac_mm_role_file WHERE role_id = $role AND file_id = $file_id";
	//$result = mysql_query($sql); // OLD CONNECTION
    // PDO DB
    $sth = $dbh->exec($sql);
    unset($sth);
...
header( "Location: admin_profile_file.php" );
?>